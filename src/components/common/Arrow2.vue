  ...
  render (h) {
    return (
      <div class={'relative arrow-wrapper'} style={`width:${this.distance}px;height:${this.boxHeight * this.current.rowSpan}px`}>
        {
          this.current.next.map((item, index) => {
            // 获取右侧指向的节点的index
            ...
            const rightSelfHeight = rightOtherHeight + (item.rowSpan || 1) * this.boxHeight / 2
            // if (['adapter01', 'adapter02', 'adapter03'].includes(this.current.name)) {
            // if (['业务系统'].includes(this.current.name)) {
            //   console.log(rightSelfHeight, rightOtherHeight, this.current.name)
            // }
            // 获取左侧当前节点上方的所有节点的高度
